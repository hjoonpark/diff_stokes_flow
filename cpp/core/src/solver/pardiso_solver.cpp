...
void PardisoSolver::Compute(const SparseMatrix& lhs) {
#ifdef PARDISO_AVAILABLE
    ia_ = nullptr;
    ja_ = nullptr;
    a_ = nullptr;
    ...
    int num_procs;
    CheckError(var, "You must provide OMP_NUM_THREADS.");
    ...
    CheckError(error == 0, "Pardiso solver: inconsistency of matrix: " + std::to_string(error));
...
#else
    PrintError("You are calling Pardiso but you haven't compiled it yet.");
#endif
}
